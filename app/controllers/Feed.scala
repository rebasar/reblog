...
object Feed extends Controller with MongoController {
  ...
  def feed(format : FeedType, tag : List[String], language: Option[Language], page: Int) = Action.async {
    ...
    cursor.collect[List](10).map { result =>
      Result(
        ...
        body = Enumerator(FeedProducer.produceFeed(format, result).getBytes)
      )
    }
  }
}
